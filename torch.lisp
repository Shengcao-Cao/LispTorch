...
(define t_tensor (lambda (data device:#n requires_grad:#f pin_memory:#f) (_torch_tensor data device requires_grad pin_memory)))
(define t_as_tensor (lambda (data device:#n) (_torch_as_tensor data device)))
(define t_from_numpy (lambda (ndarray) (_torch_from_numpy ndarray)))
(define t_zeros (lambda (*sizes out:#n layout:t_strided device:#n requires_grad:#f) (_torch_zeros *sizes out layout device requires_grad)))
(define t_ones (lambda (*sizes out:#n layout:t_strided device:#n requires_grad:#f) (_torch_ones *sizes out layout device requires_grad)))
(define t_arange (lambda (start:0 end step:1 out:#n layout:t_strided device:#n requires_grad:#f) (_torch_arange start end step out layout device requires_grad)))
(define t_range (lambda (start:0 end step:1 out:#n layout:t_strided device:#n requires_grad:#f) (_torch_range start end step out layout device requires_grad)))
(define t_eye (lambda (n m:#n out:#n layout:t_strided device:#n requires_grad:#f) (_torch_eye n m out layout device requires_grad)))
(define t_empty (lambda (*sizes out:#n layout:t_strided device:#n requires_grad:#f pin_memory:#f) (_torch_empty *sizes out layout device requires_grad pin_memory)))
(define t_full (lambda (size fill_value out:#n layout:t_strided device:#n requires_grad:#f) (_torch_full size fill_value out layout device requires_grad)))
...
(define t_rand (lambda (*sizes out:#n layout:t_strided device:#n requires_grad:#f) (_torch_rand *sizes out layout device requires_grad)))
(define t_randint (lambda (low:0 high size out:#n layout:t_strided device:#n requires_grad:#f) (_torch_randint low high size out layout device requires_grad)))
(define t_randn (lambda (*sizes out:#n layout:t_strided device:#n requires_grad:#f) (_torch_randn *sizes out layout device requires_grad)))
...
(define t_argmax (lambda (input dim) (_torch_argmax input dim)))
(define t_argmin (lambda (input dim out:#n) (_torch_argmin input dim out)))
(define t_cumprod (lambda (input dim out:#n) (_torch_cumprod input dim out)))
(define t_cumsum (lambda (input dim out:#n) (_torch_cumsum input dim out)))
(define t_dist (lambda (input other p:2) (_torch_dist input other p)))
(define t_logsumexp (lambda (input dim out:#n) (_torch_logsumexp input dim out)))
(define t_mean (lambda (input dim out:#n) (_torch_mean input dim out)))
(define t_median (lambda (input dim:-1 values:#n indices:#n) (_torch_median input dim values indices)))
(define t_mode (lambda (input dim:-1 values:#n indices:#n) (_torch_mode input dim values indices)))
(define t_norm (lambda (input p:"fro" dim:#n out:#n) (_torch_norm input p dim out)))
(define t_prod (lambda (input dim) (_torch_prod input dim)))
(define t_std (lambda (input dim unbiased:#t out:#n) (_torch_std input dim unbiased out)))
(define t_sum (lambda (input dim) (_torch_sum input dim)))
...
(define t_var (lambda (input dim unbiased:#t out:#n) (_torch_var input dim unbiased out)))
...
(define t_eq (lambda (input other out:#n) (_torch_eq input other out)))
...
(define t_max (lambda (input dim out:#n) (_torch_max input dim out)))
(define t_min (lambda (input dim out:#n) (_torch_min input dim out)))
...
(define t_bartlett_window (lambda (window_length periodic:#t layout:t_strided device:#n requires_grad:#f) (_torch_bartlett_window window_length periodic layout device requires_grad)))
(define t_blackman_window (lambda (window_length periodic:#t layout:t_strided device:#n requires_grad:#f) (_torch_blackman_window window_length periodic layout device requires_grad)))
(define t_hamming_window (lambda (window_length periodic:#t alpha:0_54 beta:0_46 layout:t_strided device:#n requires_grad:#f) (_torch_hamming_window window_length periodic alpha beta layout device requires_grad)))
...
(define t_utils_data_DataLoader (lambda (dataset batch_size:1 shuffle:#f sampler:#n batch_sampler:#n num_workers:0 pin_memory:#f drop_last:#f timeout:0 worker_init_fn:#n) (_torch_utils_data_DataLoader dataset batch_size shuffle sampler batch_sampler num_workers pin_memory drop_last timeout worker_init_fn)))
(define t_optim_SGD (lambda (params lr momentum:0 dampening:0 weight_decay:0 nesterov:#f) (_torch_optim_SGD params lr momentum dampening weight_decay nesterov)))
(define t_optim_Adam (lambda (params lr:0_001 eps:1e-08 weight_decay:0 amsgrad:#f) (_torch_optim_Adam params lr eps weight_decay amsgrad)))
(define t_optim_Adagrad (lambda (params lr:0_01 lr_decay:0 weight_decay:0 initial_accumulator_value:0) (_torch_optim_Adagrad params lr lr_decay weight_decay initial_accumulator_value)))
(define t_nn_Sequential (lambda (*args) (_torch_nn_Sequential *args)))
(define t_nn_Conv2d (lambda (in_channels out_channels kernel_size stride:1 padding:0 dilation:1 groups:1 bias:#t) (_torch_nn_Conv2d in_channels out_channels kernel_size stride padding dilation groups bias)))
(define t_nn_ReLU (lambda (inplace:#f) (_torch_nn_ReLU inplace)))
(define t_nn_MaxPool2d (lambda (kernel_size stride:#n padding:0 dilation:1 return_indices:#f ceil_mode:#f) (_torch_nn_MaxPool2d kernel_size stride padding dilation return_indices ceil_mode)))
(define tv_transforms_Compose (lambda (transforms) (_torchvision_transforms_Compose transforms)))
(define tv_transforms_ToTensor (lambda () (_torchvision_transforms_ToTensor )))
(define tv_transforms_Normalize (lambda (mean std inplace:#f) (_torchvision_transforms_Normalize mean std inplace)))
(define t_nn_Linear (lambda (in_features out_features bias:#t) (_torch_nn_Linear in_features out_features bias)))
(define t_nn_LogSoftmax (lambda (dim:#n) (_torch_nn_LogSoftmax dim)))
(define t_nn_functional_nll_loss (lambda (input target weight:#n size_average:#n ignore_index:-100 reduce:#n) (_torch_nn_functional_nll_loss input target weight size_average ignore_index reduce)))
